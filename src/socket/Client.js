...
class ClientSocket  {
    ...
    async onTyping(data){
        //consolelog('typing event received:', data);
   ...
   /*   async onDisconnect(){
        //consolelog(`User disconnected: ${socket.id}, ClienteID: ${socket.clienteId || 'Anonymous'}`);
        if (socket.clienteId) {
          connectedUsers.delete(socket.clienteId.toString());
          //consolelog('Connected clients after disconnect:', Array.from(connectedUsers.keys()));
        ...
        if (socket.userId) {
          ...
          //consolelog('Connected users after disconnect:', Array.from(connectedUsers.keys()));
    ...
    async onJoinRoom(data){
      ...
      async onSendMessage(data){
        //consolelog("sendMessage",data)
        // data should include: { receiverId: string (clienteId), content: string }
        //consolelog('sendMessage event received:', data, 'from clienteId:', socket.clienteId);
   ...
   */
    ...
    async onDisconnect(){
        ...
        if(this.events.clienteRemoved)this.events.clienteRemoved(this.socket.clienteId)
        if(this.events.clientDisconnected)this.events.clientDisconnected(this.socket.clienteId)
        //consolelog("client disconnected ",this.socket.clienteId)
    }
